...
| Step | Act | Check |
| --- | --- | --- |
| 0 | Land what is in flight: the S1.5–S1.7 seat's delivery here (streaming, SHA-224, assurance), and the coordinator commits foldlab's S2 tree after its own review. Nothing is extracted from an uncommitted tree. | both repositories green at the commits the extraction will read |
