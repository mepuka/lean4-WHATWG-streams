...
coverage number: `WhatwgStreamsTest/Audit/SpecCoverage.lean` owns coverage
...
Implementation fence: `WhatwgStreams/Data/Queue.lean`,
...
`WhatwgStreamsTest/Counterexamples/Data/Queue.lean`.
...
- option (ii) cannot be Lean's `Float`. `Float` operations are opaque `extern`
  ...
  `WhatwgStreamsTest/Audit/AxiomGate.lean`. Option (ii) means a **modelled**
...
| construction | `required-open` | D0 through D8 are frozen by name and signature in `WhatwgStreamsTest/Data/QueueContract.lean`; the module that declares them does not exist yet. Closes when the builder lands the fence and the battery goes green |
...
| counterexamples | `required-closed` | `WS-DATA-CE-001` .. `WS-DATA-CE-010`, all `SEEDED` with proved green witnesses in `WhatwgStreamsTest/Counterexamples/Data/Queue.lean`; forty-four theorems, receipts `none` or `propext`. They close when the ruled instance rejects each attack |
...
| trust | `required-open` | the counterexample module's receipts are measured and inside the semantic ceiling; `lake exe trustselftest` passes with the declared red set matching the observed red set exactly. **Open:** the ninety-nine production receipts, which arrive with the builder in `WhatwgStreamsTest/Data/QueueAxiomReport.lean` |
| coverage | `required-open` | the per-row map in section 6 is a proposal. No coverage state changes until the builder's witnesses land and `WhatwgStreamsTest/Audit/SpecCoverage.lean` is edited under its own claim, as `docs/SPEC-COVERAGE.md` requires |
...
`WhatwgStreamsTest/Audit/SpecCoverage.lean`; the authority for the numbers is
