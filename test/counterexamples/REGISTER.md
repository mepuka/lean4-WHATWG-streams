...
| --- | --- | --- | --- | --- |
| `WS-SHA-CE-001` | `CLOSED` | `Sha256.Spec.H0` is FIPS 180-4 §5.3.3 and not §5.3.2 | `WhatwgTest/Streams/Counterexamples/Sha/Mutants.lean`, `ce001_sha224IV` with its control `ce001_control`; `Sha256.Bridge.sha256_ne_sha224_iv` on the constants | none: the shipped `H0` is §5.3.3, and the witness pins that the choice is load-bearing |
